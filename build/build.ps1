...
if (-not (Test-Path -LiteralPath $ProjPath)) {
    Write-Host "Project file not found: $ProjPath" -ForegroundColor Red
    exit 1
}
...
foreach ($rid in $rids) {
    ...
    if ($running) {
        Write-Host "Stopping running process: $BaseName" -ForegroundColor Yellow
    ...
    if (Test-Path -LiteralPath $srcExePath) {
        ...
        if ($exe) {
            ...
            Write-Host "No exe found in $OutDir" -ForegroundColor Yellow
        }
    }
}
